...
class VirtualStickFragment : DJIFragment(), CompoundButton.OnCheckedChangeListener {
    ...
    // ================= 新增：摇杆控制的默认速度设置 =================
    private val defaultHorizontalSpeed = 0.05 // 水平移动速度基准值 (m/s)
    private val defaultVerticalSpeed = 0.05   // 垂直升降速度基准值 (m/s)
    private val defaultYawSpeed = 1.0       // 偏航旋转速度基准值 (°/s)
    // ==========================================================
     ...
     */
    /**
     * 初始化大疆飞行器状态实时监听 (适配 MSDK V5 规范)
     */
    private fun initFlightDataListener() {
        // 1. 监听高度 (Altitude) -> 使用 FlightControllerKey.KeyAltitude
        KeyManager.getInstance().listen(KeyTools.createKey(FlightControllerKey.KeyAltitude), this) { _, newValue ->
            newValue?.let { altitude ->
                mainHandler.post {
                    val formattedAlt = String.format("%.1f m", altitude)
                    binding?.tvFlightAltitude?.text = formattedAlt
                    binding?.tvAltitude?.text = formattedAlt // 同步更新右摇杆中间的高度显示
                }
            }
        }

        // 2. 监听速度 (Velocity3D) -> 使用 FlightControllerKey.KeyAircraftVelocity
        KeyManager.getInstance().listen(KeyTools.createKey(FlightControllerKey.KeyAircraftVelocity), this) { _, newValue ->
            newValue?.let { velocity ->
                // MSDK V5 返回的是 Velocity3D 对象，包含 x, y, z 三轴速度
                val speed = kotlin.math.sqrt(velocity.x * velocity.x + velocity.y * velocity.y)
                mainHandler.post {
                    val formattedSpeed = String.format("%.1f m/s", speed)
                    binding?.tvFlightSpeed?.text = formattedSpeed
                    binding?.tvHorizontalSpeed?.text = formattedSpeed // 同步更新左摇杆中间的速度显示
                }
            }
        }

        // 3. 监听电池剩余电量百分比 -> 使用 BatteryKey.KeyChargeRemainingInPercent
        KeyManager.getInstance().listen(KeyTools.createKey(BatteryKey.KeyChargeRemainingInPercent), this) { _, newValue ->
            newValue?.let { percent ->
                mainHandler.post {
                    // 假设一块电池飞30分钟(1800秒)，粗略估算剩余分钟数
                    val estimatedMinutes = (percent / 100.0) * 30.0
                    binding?.tvFlightTime?.text = String.format("%d%% (约%d分)", percent, estimatedMinutes.toInt())

                    // 电量过低变红警告
                    if (percent <= 20) {
                        binding?.tvFlightTime?.setTextColor(android.graphics.Color.RED)
                    } else {
                        binding?.tvFlightTime?.setTextColor(android.graphics.Color.GREEN)
                    }
                }
            }
        }
    ...
    /**
     * 绑定摇杆按钮事件 (按下触发动作，松开归零)
     */
    /**
     * 绑定摇杆按钮事件 (按下触发动作，松开归零)，并联动当前的速度等级
     */
    @SuppressLint("ClickableViewAccessibility")
    private fun initDirectionButtons() {

        // 动态获取当前水平速度 (如果 ViewModel 中有值则使用，否则用默认值 1.0)
        val getHorizontalSpeed = { virtualStickVM.currentSpeedLevel.value ?: defaultHorizontalSpeed }

        // 垂直升降速度 (通常不需要像水平飞那么快，可以适当给个比例，或者直接用独立默认值)
        // 这里假设垂直速度等于水平速度的一半，如果想独立控制可以改回 defaultVerticalSpeed
        val getVerticalSpeed = { (virtualStickVM.currentSpeedLevel.value ?: defaultVerticalSpeed) * 0.5 }

        // 左侧控制：水平移动 (Pitch 和 Roll)
        setupButtonTouch(binding?.btnUp) { isPressed -> pitch = if (isPressed) getHorizontalSpeed() else 0.0 }
        setupButtonTouch(binding?.btnDown) { isPressed -> pitch = if (isPressed) -getHorizontalSpeed() else 0.0 }
        setupButtonTouch(binding?.btnLeft) { isPressed -> roll = if (isPressed) -getHorizontalSpeed() else 0.0 }
        setupButtonTouch(binding?.btnRight) { isPressed -> roll = if (isPressed) getHorizontalSpeed() else 0.0 }

        // 右侧控制：高度和偏航 (Throttle 和 Yaw)
        setupButtonTouch(binding?.btnRise) { isPressed -> verticalThrottle = if (isPressed) getVerticalSpeed() else 0.0 }
        setupButtonTouch(binding?.btnDescend) { isPressed -> verticalThrottle = if (isPressed) -getVerticalSpeed() else 0.0 }

        // 偏航(旋转)的单位是 度/秒(°/s)，和水平速度的 米/秒(m/s) 概念不同，建议保持独立设定，比如恒定 30°/s
        setupButtonTouch(binding?.btnTurnLeft) { isPressed -> yaw = if (isPressed) -defaultYawSpeed else 0.0 }
        setupButtonTouch(binding?.btnTurnRight) { isPressed -> yaw = if (isPressed) defaultYawSpeed else 0.0 }
        ...
        binding?.btnEmergencyStop?.setOnClickListener {
            ...
            ToastUtils.showToast("已触发急停悬停！")
        }
    }

    @SuppressLint("ClickableViewAccessibility")
    private fun setupButtonTouch(button: Button?, action: (Boolean) -> Unit) {
